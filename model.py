...
class UnifiedFramework(nn.Module):
    def __init__(self, config):
        ...
        self.depend_embedding = nn.Embedding(config['depend_set_size'], config['depend_embedding_dim'])
        ...
        self.generic_path_repr_backward = nn.LSTM(config['feature_size'], config['gpr_hidden_size'],
                                                  config['gpr_num_layers'], bidirectional=False, batch_first=True,
                                                  dropout=config['drop_out'])
        self.relation_path_repr_forward = nn.LSTM(config['depend_embedding_dim'], config['rpr_hidden_size'],
                                                  config['rpr_num_layers'], bidirectional=False, batch_first=True,
                                                  dropout=config['drop_out'])
        self.relation_path_repr_backward = nn.LSTM(config['depend_embedding_dim'], config['rpr_hidden_size'],
                                                   config['rpr_num_layers'], bidirectional=False, batch_first=True,
                                                   dropout=config['drop_out'])
        self.word_lex_repr_weight = nn.Linear(config['word_embedding_dim'] * 2 + config['pos_embedding_dim'],
                                              config['feature_size'])
